...
from django_admin_smart_filters.declarations import normalize_declarations
from django_admin_smart_filters.query import apply_filter_state
...
class SmartFilterAdminMixin:
    ...
    def changelist_view(
        ...
        labels = {
            spec.field_name: spec.field_name.replace("_", " ").title() for spec in specs
        }
        ...
        context["smart_filter_controls_html"] = self.render_smart_filter_controls(
            controls, theme_adapter=theme_adapter
        )
        ...
        return super().changelist_view(request, extra_context=context)  # type: ignore[misc]
    ...
    def smart_filter_autocomplete_view(self, request: HttpRequest) -> JsonResponse:
        ...
        if target_spec is None:
            return JsonResponse(
                {
                    "error": f"invalid autocomplete field: {field_selector or '(missing)'}"
                },
                status=400,
            )
        ...
        try:
            autocomplete_request = parse_autocomplete_request(
                target_spec, request.GET, state=scoped_state
            )
    ...
    def render_smart_filter_controls(
        ...
        *,
        ...
        adapter = resolve_theme_adapter(
            theme_adapter or self.get_smart_filter_theme_adapter()
        )
        return render_to_string(
            adapter.controls_template, {"filter_controls": controls}
        )
