...
def build_remove_one_url(
    current_query: QueryDict, target_chip: Mapping[str, Any]
) -> str:
...
def _chips_for_spec(spec: FilterSpec, title: str, value: Any) -> list[dict[str, Any]]:
    ...
    if spec.filter_kind in {"date_range", "numeric_range"} and isinstance(
        value, Mapping
    ):
        if spec.filter_kind == "date_range":
            ...
            keys = [
                f"{spec.param_name}_start",
                f"{spec.param_name}_end",
                f"{spec.param_name}_min",
                f"{spec.param_name}_max",
            ]
        else:
