...
if TYPE_CHECKING:
    from django_admin_smart_filters.builder import BuilderFilterDeclaration

from django_admin_smart_filters.contracts import FilterSpec, QueryHook, WidgetHook
from django_admin_smart_filters.params import resolve_param_name
from django_admin_smart_filters.registry import resolve_filter_component
from django_admin_smart_filters.validation import (
...
def DropdownFilter(
    field_name: str, *, alias: str | None = None
) -> ClassFilterDeclaration:
    return ClassFilterDeclaration(
        field_name=field_name, filter_kind="dropdown", alias=alias
    )
...
def normalize_builder_declaration(
    declaration: "BuilderFilterDeclaration",
) -> FilterSpec:
