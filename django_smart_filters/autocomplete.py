...
from django_smart_filters.validation import validate_filter_spec
...
def parse_autocomplete_request(
    ...
    *,
    ...
    if spec.filter_kind != "autocomplete":
        raise ValueError(f"Invalid autocomplete spec '{spec.field_name}': filter kind must be 'autocomplete'.")
    ...
    requested_limit = _parse_positive_int(params.get("limit"), "limit", default=DEFAULT_AUTOCOMPLETE_PAGE_SIZE)
...
def search_autocomplete_options(queryset: Any, request: AutocompleteRequest) -> AutocompleteResultPage:
    """Run server-side autocomplete search with deterministic pagination."""

    if len(request.query) < MIN_AUTOCOMPLETE_QUERY_LENGTH:
        return AutocompleteResultPage(results=[], page=request.page, limit=request.limit, has_next=False)
    ...
    return AutocompleteResultPage(results=results, page=request.page, limit=request.limit, has_next=has_next)
