...
def apply_filter_state(queryset: Any, specs: Iterable[FilterSpec], state: Mapping[str, Any]) -> Any:
...
def _normalize_numeric_range(value: Any, param_name: str) -> dict[str, float | None] | None:
    if value is None:
        return None
    if not isinstance(value, Mapping):
