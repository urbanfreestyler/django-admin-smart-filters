...
from django_admin_smart_filters.validation import FilterValidationError
...
def test_invalid_field_name_raises_at_normalization_time() -> None:
    ...
    with pytest.raises(FilterValidationError, match="non-empty field name"):
        normalize_class_declaration(
            ClassFilterDeclaration(field_name=None, filter_kind="dropdown")  # type: ignore[arg-type]
        )
...
def test_unsupported_filter_kind_message_includes_invalid_and_supported_values() -> (
    None
):
    with pytest.raises(
        FilterValidationError, match="Unsupported filter_kind 'unknown_kind'"
    ) as exc_info:
        normalize_class_declaration(
